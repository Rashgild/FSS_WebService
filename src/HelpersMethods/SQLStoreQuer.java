...
public class SQLStoreQuer {
    ...
    public static String Query_SkeletonSelect()
    {
        return  "select\n" +
                ...
                " Order by DDID "+
                "limit "+GlobalVariables.limit+" Offset 0";
    }
    ...
    public static String Query_Treats()
    {
        return  "select\n" +
                ...
                "                left join patient vkname on vkname.id = w2.person_id\n" +
                "                left join VocWorkFunction vwf2 on vwf2.id = wf2.workFunction_id\n" +
                "                where dd.issuedate between '"+GlobalVariables.DateFrom+"' and '"+GlobalVariables.DateTo+"'";
               // "where p.id = 343577";
    }
     public static String forXMLFileLnLpuN_One()
     {
         String str = "select\n" +
                 "dd.issuedate, \n" +
                 "dd.id as DDID,    \n" +
     ...
     public static String forXMLFileLnLpuN_Two()
     {
         return "select\n" +
                 "dd.id as DDID,    \n" +
                 "p.lastname as SURNAME,\n" +
                 ...
                 "left join patient vkname on vkname.id = w2.person_id\n" +
                 "left join VocWorkFunction vwf2 on vwf2.id = wf2.workFunction_id\n" +
                 "where dd.id = "+GlobalVariables.DisabilityDocument_id;
     }
}
